...
for i in range(1, 23):
    print i
    with gzip.open("/mnt/lab_data/montgomery/shared/1KG/hg38/ALL.chr{0}_GRCh38.genotypes.20170504.vcf.gz".format(i)) as f:
        for line in f:
            if line.startswith("#"):
                continue
...
for tissue_dir in glob.glob("/users/mgloud/projects/rna_editing/data/eqtls/*"):
    tissue = tissue_dir.split("/")[-1]

    with gzip.open("/users/mgloud/projects/rna_editing/data/eqtls/{0}/{0}.edMat.20cov.60samps.noXYM.qqnorm.bed.QTLtools.nominal.with.alleles.txt.gz".format(tissue), "w") as w:
        with gzip.open("/users/mgloud/projects/rna_editing/data/eqtls/{0}/{0}.edMat.20cov.60samps.noXYM.qqnorm.bed.QTLtools.nominal.txt.gz".format(tissue)) as f:
            for line in f:
                data = line.strip().split()
