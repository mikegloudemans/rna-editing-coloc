...
def main():
    ...
    for i in range(len(kept_data)):
        ...
        # NOTE: Right now we're using estimated sample sizes for GWAS and eQTL studies.
        # To do this properly, we'll really need to load the actual sample numbers instead,
        # or to prove that they really make no difference.
        ...
        with open("/users/mgloud/projects/rna_editing/tmp/edqtl_aggro_config{0}.config".format(i), "w") as w:
            json.dump(temp, w)

        # Run the test
        subprocess.call("python /users/mgloud/projects/brain_gwas/scripts/dispatch.py /users/mgloud/projects/rna_editing/tmp/edqtl_aggro_config{0}.config 1 &".format(i), shell=True)

        while int(subprocess.check_output('''ps -ef | grep "python /users/mgloud/projects/brain_gwas/scripts/dispatch.py /users/mgloud/projects/rna_editing/tmp/edqtl_aggro_config" | wc -l''', shell=True)) > 8:
            time.sleep(5)

template = '''
{
        "out_dir_group": "rna-editing-aggro-tests",
