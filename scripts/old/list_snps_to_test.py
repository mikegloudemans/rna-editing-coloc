...
def main():
    ...
    with open("/users/mgloud/projects/rna_editing/output/snps_to_test_gwas1e6_eqtl1e6.txt", "w") as w:
        ...
        for file in sorted(files):
            ...
            for snp in info:
                ...
                for tissue in glob.glob("/users/mgloud/projects/rna_editing/data/tabix_eqtls/*.gz"):
                    ...
                    for match in matches:
                        ...
                        if float(data[11]) < 1e-6:
                            w.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n".format(snp[0], snp[1], file, tissue, snp[3], snp[2], data[11], data[0]))
                            w.flush()
