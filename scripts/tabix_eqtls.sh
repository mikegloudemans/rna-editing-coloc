...
for f in `ls /users/mgloud/projects/rna_editing/data/eqtls_aggro`; do
	filebase=`echo "$f" | sed s/.edMat.20cov.60samps.noXYM.qqnorm.bed.QTLtools.nominal.Fisher_combined.txt.gz//g`
	zcat /users/mgloud/projects/rna_editing/data/eqtls_aggro/$f | head -n 1 > /users/mgloud/projects/rna_editing/data/tabix_eqtls_aggro/$filebase.Fisher.combined.sorted.txt
	zcat $filebase.edMat.20cov.60samps.noXYM.qqnorm.bed.QTLtools.nominal.Fisher_combined.with.alleles.txt.gz | sort -k3,3 -k4,4n > /users/mgloud/projects/rna_editing/data/tabix_eqtls_aggro/$filebase.Fisher.combined.sorted.txt
